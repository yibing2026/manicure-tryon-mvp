...
自动质检由项目脚本 `scripts/evaluate_tryon_results.py` 完成，不依赖人工逐张判断。它读取批量生成结果和官方样例信息，对每张试戴图给出质量状态与问题原因：
...
v1 质检是规则型评估，优点是可解释、成本低、可离线运行；边界是不能精确识别所有指甲贴合偏移。后续可以接入手部关键点检测、指甲分割模型或多模态大模型评分，提高对位置偏差、覆盖度和风格一致性的判断能力。
...
运营日报通过 `npm run report:ops` 生成，输出趋势洞察、主推款式、行动建议和风险提示。当前版本不声称使用真实用户行为数据；正式上线后应将 mock 热度替换为真实埋点数据，包括曝光、点击、收藏、试戴、加购、预约和成交。
